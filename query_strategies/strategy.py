...
class Strategy:
    ...
    def train(self):
        ...
        if self.args['net_type'] == 'pretrained':
            self.clf = self.args['pretrained_resnet']().cuda()
        elif self.args['net_type'] == 'fedacross':
            self.clf = self.args['pretrained_fedacross']().cuda()
        else:
        ...
        epoch = 1
        accCurrent = 0.
        while accCurrent < self.args["train_acc"]: 
    ...
    def load_model(self, path):
        self.clf = self.args['pretrained_fedacross']().cuda()
        self.clf.load_state_dict(torch.load(path))
        print("Model loaded")
    ...
    def get_exp_grad_embedding(self, X, Y, probs=[], model=[], rank=1):
        if type(model) == list:
            model = self.clf
        embDim = model.get_embedding_dim()
        model.eval()
